...
if(isset($_POST['hod_signup_btn'])) {
    ...
    if (empty_input_func($fname, $lname, $email, $school_id, $password)) {
        ...
        $sql = "SELECT email, school_id, faculty FROM hod_tb WHERE email = ?  OR school_id = ? OR faculty = ?";
        ...
        if(mysqli_num_rows($result) > 0) {
            $response['error']= "An HOD with same email address or school id or faculty is already registered";
        } else {

                $sql = "INSERT INTO hod_tb(fname, lname, email, school_id, faculty,  password) 
                VALUES (?, ?, ?, ?, ?, ?)";
                mysqli_stmt_prepare($stmt, $sql);
                mysqli_stmt_bind_param($stmt, "ssssss", $fname, $lname, $email, $school_id, $faculty, $hashed_pwd);
                if(mysqli_stmt_execute($stmt)) {
                    $sql = "SELECT id, email, school_id FROM hod_tb WHERE email = ?  OR school_id = ?";
                    mysqli_stmt_prepare($stmt, $sql);
