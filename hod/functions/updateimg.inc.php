...
if (isset($_SESSION['hodID'])) {
    ...
    $sql = "SELECT image FROM hod_tb WHERE school_id = '{$school_id}'";
    ...
    if ($imgFileName !== '') {
        if (in_array($fileExt, $allowedExtensions) && $imgFileError === 0 && $imgFileSize <= 3145728) {
            $id = time() + rand(1, 1000);
            $imgFileName = 'hod' . $id . '.' . $fileExt;

            if ($row['image'] !== null) {
                unlink("../../uploads/hod/" . $row['image']);
            }

            // Compress and save the image
            $compressedImagePath = '../../uploads/hod/' . $imgFileName;
            if (compressAndSaveImage($imgFileTmpName, $compressedImagePath, 50)) {
                $sql = "UPDATE hod_tb SET image = '{$imgFileName}' WHERE school_id = '{$school_id}'";
